...
sealed class SpawnPlayerSystems : IEcsRunSystem
{
    ...
    void IEcsRunSystem.Run()
    {
        ...
        foreach (var e in photonPlayers)
        {
            ...
            entity.Get<HealthPointComponent>().Value = entity.Get<HealthPointComponent>().MaxValue;
            ...
            foreach (var b in battleFilter)
            {
                ...
                if (isSpawned)
                {
                    ...
                    player.view.OnDamage(entity.Get<HealthPointComponent>().MaxValue);

                    if (player.view.photonView.IsMine)
                    {
                        var pos = spawn.pos + GetRandomOffset();
                        player.view.transform.position = new Vector3(pos.x, 0, pos.y);

                        var navMesh = player.view.GetComponent<UnityEngine.AI.NavMeshAgent>();
                        if (navMesh) navMesh.enabled = true;
                    }

                    var renderers = player.view.transform.GetComponentsInChildren<SpriteRenderer>();
